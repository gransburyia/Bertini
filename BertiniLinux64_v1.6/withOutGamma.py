...
def makeHomotopyFile(nValue):
    ...
    hFile.write("h_t =  t * ( ( b - a ) / n + 1 ) + ( 1 - t ) * ( ( b - a ) / n + 2 );\n\n")
    
    hFile.write("y_1 = ( 1 - t ) * y" + str(nValue + 1) + " + beta * t;\n\n")

    hFile.write("y_2 = beta * ( 1 - t );\n\n")
...
def combineSolutions(yN, ssSol, finalSol, nValue):
    if(nValue == 1):
        ...
        for k in ssSol:
            ...
            print("This is beginning index", beginningIndex)
            print("This is yN index:", dNSol.index(yN))
            while(beginningIndex <= dNSol.index(yN)):
                solutionString += dNSol[beginningIndex]
                beginningIndex += 1
            solutionString += k
            print("This is solution string: ", solutionString)
...
def writeEquations(nValue, fileObject):
    tempValue = nValue
    while(tempValue > 1):
        newEquation = p.replace("y", "y" + str(nValue))
        fileObject.write("f" + str(nValue - 2) + " = y" + str(nValue -2) + " - 2 * y" + str(nValue - 1) + " + y" + str(nValue) + " - (h_t) ^ 2 *(" + newEquation + ");\n")
        tempValue = tempValue - 1
    Y_1Equation = p.replace("y", "y" + str(nValue))
    fileObject.write("f" + str(nValue -1) + " = y" + str(nValue - 1) + " - 2 * y" + str(nValue) + " + y_1 - (h_t) ^ 2 *(" + Y_1Equation + ");\n")
    Y_2Equation = p.replace("y", "y" + str(nValue + 1))
    fileObject.write("f" + str(nValue) + " = y" + str(nValue) + " - 2 * y" + str(nValue + 1) + " + y_2 - (h_t) ^ 2 * (" + Y_2Equation + ");\n\n") 
...
print("These are read in from d1: ", dNSol)
